...
export default function SettingsPage() {
  ...
  const changePassword = async (e) => {
    ...
    if (!passwordForm.currentPassword || !passwordForm.newPassword) {
      toast("Both password fields are required", "warning");
      return;
    }
        ...
        <div className="space-y-6">
          ...
          <div className="relative overflow-hidden rounded-2xl border border-slate-800 bg-gradient-to-br from-slate-900 via-slate-900 to-slate-950 p-6">
            ...
            <div className="flex items-center gap-4">
              <div
                className={`flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br text-2xl font-black text-white shadow-lg ${avatarGrad}`}
              >
                {initials}
              </div>
              <div>
                ...
                <div className="mt-1 flex items-center gap-2">
                  <span className="inline-flex items-center rounded-full border border-violet-500/30 bg-violet-500/15 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-violet-300">
                    {user.role}
                  </span>
                  {user.role === "admin" && (
                    <button
                      className="inline-flex items-center rounded-full border border-cyan-500/30 bg-cyan-500/10 px-2 py-0.5 text-[10px] font-semibold text-cyan-300 hover:bg-cyan-500/20 transition"
          ...
          <SettingsSection title="Account Details" icon="ℹ">
            <div className="grid gap-4 sm:grid-cols-3">
              {[
                ...
                >
                  <p className="mb-1 text-[11px] font-semibold uppercase tracking-widest text-slate-600">
                    {item.label}
                  </p>
          ...
          <div className="rounded-2xl border border-rose-500/20 bg-rose-500/5 p-5">
            ...
            <p className="mb-4 text-xs text-slate-500">
              Signing out will end your current session and you will need to log
              in again.
            </p>
