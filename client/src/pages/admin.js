...
export default function Admin() {
  ...
  const currentCount = activeTab === "users" ? filteredUsers.length : filteredProjects.length;
      ...
      <div className="layout dashboard-layout">
        <section className="left dashboard-left">
          ...
          <div className="hero-text">
            ...
            <p className="hero-sub">System-wide moderation and visibility for platform operations.</p>
          </div>
          ...
          <div className="stats">
            ...
            <div className="stat">
              <div className="stat-num">{analytics?.totalActiveProjects ?? 0}</div>
        ...
        <Navbar title="Admin Control Center" showDashboard onLogout={logout} />

        <section className="panel-card mb-6 p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-sm text-slate-300">Control panel for moderation and system operations</p>
              <p className="text-lg font-semibold text-white">
                Signed in as {user.name} <span className="text-cyan-300">({user.role})</span>
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button className="btn-secondary" onClick={() => router.push("/dashboard?preview=member")}>
                <span className="inline-flex items-center gap-2">
                  <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        {analytics ? (
          <section className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <div className="glass-card p-4">
              <p className="text-xs uppercase tracking-wide text-slate-300">Total Users</p>
              <p className="mt-1 text-3xl font-bold text-cyan-300">{analytics.totalUsers}</p>
            </div>
            <div className="glass-card p-4">
              <p className="text-xs uppercase tracking-wide text-slate-300">Total Projects</p>
              <p className="mt-1 text-3xl font-bold text-emerald-300">{analytics.totalProjects}</p>
            </div>
            <div className="glass-card p-4">
              <p className="text-xs uppercase tracking-wide text-slate-300">Active Projects</p>
              <p className="mt-1 text-3xl font-bold text-blue-300">{analytics.totalActiveProjects ?? 0}</p>
            </div>
            <div className="glass-card p-4">
              <p className="text-xs uppercase tracking-wide text-slate-300">Admin Accounts</p>
        ...
        {analytics ? (
          <section className="panel-card mb-6 p-5">
            <h3 className="mb-4 text-lg font-semibold text-white">System Overview</h3>
            <div className="h-56">
              <Bar
                data={{
        ...
        <section className="panel-card p-5">
          ...
          <input
            ...
            placeholder={activeTab === "users" ? "Search users by name, email, role" : "Search projects by title or description"}
            value={searchQuery}
              ...
              {filteredUsers.map((u) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs uppercase tracking-wide text-slate-500">{u.role}</p>
                  </div>
