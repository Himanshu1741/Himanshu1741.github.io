...
export default function ProjectSettingsPage() {
  ...
  const [settingsForm, setSettingsForm] = useState({ title: "", description: "", github_repo: "" });
  ...
  const canEditProject = isCreator || projectPermissions.can_change_project_name;
  ...
  const normalizeGithubRepoUrl = (value) => {
    ...
    const directUrl = raw.match(/^https?:\/\/(?:www\.)?github\.com\/([^/]+)\/([^/]+?)(?:\.git|\/)?$/i);
    if (directUrl) {
      return `https://github.com/${directUrl[1]}/${directUrl[2]}`;
    }

    const hostPath = raw.match(/^(?:www\.)?github\.com\/([^/]+)\/([^/]+?)(?:\.git|\/)?$/i);
    if (hostPath) {
      return `https://github.com/${hostPath[1]}/${hostPath[2]}`;
    }

    const ownerRepo = raw.match(/^([^/\s]+)\/([^/\s]+)$/);
    if (ownerRepo) {
      return `https://github.com/${ownerRepo[1]}/${ownerRepo[2]}`;
    }
  ...
  const updateMemberPermission = async (memberId, key, value) => {
    try {
      const res = await API.put(`/projects/${id}/members/${memberId}/permissions`, {
        [key]: value
      });
      ...
      alert(error?.response?.data?.message || "Failed to update member permission");
    }
  };
  ...
  return (
    <main className="login-shell">
      <div className="layout dashboard-layout">
        <section className="left dashboard-left">
          ...
          <div className="hero-text">
            ...
            <p className="hero-sub">
              Manage members, roles, permissions, status, and GitHub repository links from one place.
            </p>
          </div>
        </section>
        ...
        <Navbar title={`Project Settings: ${project?.title || id}`} showDashboard onLogout={logout} />

        <div className="mb-6">
          <button className="btn-secondary w-full sm:w-auto" onClick={() => router.push(`/project/${id}`)}>
        ...
        {canAddMembers ? (
          <div className="panel-card mb-6 p-5">
            <h3 className="mb-3 text-lg font-semibold text-white">Add Member</h3>
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                <label className="flex items-center gap-2 text-xs text-slate-200">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewMemberConfig((prev) => ({ ...prev, member_role: e.target.value }))
                    }
                  />
                </label>
                <label className="flex items-center gap-2 text-xs text-slate-200">
                  <input
                    ...
                    onChange={(e) =>
                      setNewMemberConfig((prev) => ({ ...prev, can_manage_tasks: e.target.checked }))
                    }
                  />
                  Manage Tasks
                </label>
                <label className="flex items-center gap-2 text-xs text-slate-200">
                  <input
                    ...
                    onChange={(e) =>
                      setNewMemberConfig((prev) => ({ ...prev, can_manage_files: e.target.checked }))
                    }
                  />
                  Manage Files
                </label>
                <label className="flex items-center gap-2 text-xs text-slate-200">
                  <input
                    ...
                    onChange={(e) =>
                      setNewMemberConfig((prev) => ({ ...prev, can_chat: e.target.checked }))
                    }
                  />
                  Chat
                </label>
                <label className="flex items-center gap-2 text-xs text-slate-200">
                  <input
                    type="checkbox"
                    checked={Boolean(newMemberConfig.can_change_project_name)}
                    onChange={(e) =>
                ...
                <label className="flex items-center gap-2 text-xs text-slate-200">
                  <input
                    ...
                    onChange={(e) =>
                      setNewMemberConfig((prev) => ({ ...prev, can_add_members: e.target.checked }))
                    }
                  />
                  Add Members
                </label>
        ...
        {canEditProject ? (
          <div className="panel-card mb-6 p-5">
            <h3 className="mb-3 text-lg font-semibold text-white">Update Project</h3>
                ...
                onClick={async () => {
                  ...
                  try {
                    const res = await API.put(`/projects/${id}`, {
                      ...
                      github_repo: normalizeGithubRepoUrl(settingsForm.github_repo)
                    });
                    setProject(res.data.project);
                    setSettingsForm((prev) => ({
                      ...prev,
                      github_repo: normalizeGithubRepoUrl(prev.github_repo)
                    }));
        ...
        {isCreator ? (
          <div className="panel-card mb-6 p-5">
            <h3 className="mb-3 text-lg font-semibold text-white">Project Status</h3>
        ...
        {isCreator ? (
          <div className="panel-card mb-6 p-5">
            <h3 className="mb-3 text-lg font-semibold text-white">Manage Members</h3>
              ...
              {members.map((m) => (
                <div key={m.id} className="rounded-lg bg-slate-800 px-3 py-2">
                  <div>
                    ...
                    <p className="text-xs text-slate-400">Role: {m.member_role || "member"}</p>
                  </div>
                    ...
                    <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                      <label className="flex items-center gap-2 text-xs text-slate-200">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setMembers((prev) =>
                              prev.map((x) => (x.id === m.id ? { ...x, member_role: nextRole } : x))
                            );
                          }}
                      ...
                      <label className="flex items-center gap-2 text-xs text-slate-200">
                        <input
                          ...
                          onChange={(e) => updateMemberPermission(m.id, "can_manage_tasks", e.target.checked)}
                        />
                        Manage Tasks
                      </label>
                      <label className="flex items-center gap-2 text-xs text-slate-200">
                        <input
                          ...
                          onChange={(e) => updateMemberPermission(m.id, "can_manage_files", e.target.checked)}
                        />
                        Manage Files
                      </label>
                      <label className="flex items-center gap-2 text-xs text-slate-200">
                        <input
                          ...
                          onChange={(e) => updateMemberPermission(m.id, "can_chat", e.target.checked)}
                        />
                        Chat
                      </label>
                      <label className="flex items-center gap-2 text-xs text-slate-200">
                        <input
                          ...
                          onChange={(e) => updateMemberPermission(m.id, "can_change_project_name", e.target.checked)}
                        />
                        Change Project Name
                      </label>
                      <label className="flex items-center gap-2 text-xs text-slate-200">
                        <input
                          ...
                          onChange={(e) => updateMemberPermission(m.id, "can_add_members", e.target.checked)}
                        />
                        Add Members
                      </label>
                      <button
