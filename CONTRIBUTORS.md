# Contributors

## Lead Developer

**Himanshu Kumar** ✨

- **Role**: Founder and Lead Developer
- **Email**: hkagarwal256@gmail.com
- **GitHub**: [Himanshu1741](https://github.com/Himanshu1741)
- **Period**: February 2026 - March 2026

### Contributions

- 🏗️ **Architecture & Design**: Designed the entire Real-Time Collab Hub architecture
- 💻 **Full Stack Development**: Built both frontend (Next.js) and backend (Node.js/Express)
- 🔐 **Authentication**: Implemented OAuth 2.0 (GitHub, Google) with Passport.js
- ⚡ **Real-time Features**: Developed WebSocket functionality for real-time communication
- 📊 **Database Design**: Designed MySQL schema with Sequelize ORM
- 🎨 **UI/UX**: Created responsive Tailwind CSS design with dark/light modes
- 📱 **Features Developed**:
  - User authentication and authorization
  - Project management system
  - Task tracking with drag-and-drop
  - Real-time messaging and notifications
  - File upload and preview
  - Analytics and reporting
  - Admin dashboard
  - Milestone tracking

---

## Copyright

© 2026 Himanshu Kumar. All rights reserved.

**License**: MIT

This project represents the intellectual work and innovation of Himanshu Kumar. While open-sourced under the MIT License, all original design and development credit belongs to the creator.

---

**Last Updated**: March 31, 2026
